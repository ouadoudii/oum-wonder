...
import type { Budget, DesignConcept, ImageSignals, Mode, RoomType } from './types.js'
...
const state: State = {
  step: 'start', imageUrl: null, fileName: '', signals: neutralSignals, mode: 'inspire', roomType: 'Wohnzimmer', budget: 'balanced', concern: '', concept: null, isAnalysing: false,
}
...
function topbar(compact = false): string {
  return `<header class="topbar${compact ? ' compact' : ''}">
    ${compact ? `<button class="icon-button" data-action="back-to-brief" aria-label="Zurück">${icon('back')}</button>` : ''}
    <div class="brand"><span class="brand-mark">O</span><span>Oum Wonder</span></div>
    ${compact ? `<button class="icon-button" data-action="reset" aria-label="Neu starten">${icon('reset')}</button>` : `<span class="brand-tagline">Räume neu denken</span>`}
  </header>`
}
...
function renderStart(): string {
  ...
  return `${topbar()}
    <section class="hero-copy">
      <div class="eyebrow">${icon('sparkle')} Renovieren beginnt mit einer besseren Idee</div>
      <h1>Zeig mir den Raum.<br><em>Ich sehe, was möglich ist.</em></h1>
      <p>Oum Wonder denkt Licht, Proportionen, Möbel, Farbe und Materialien als ein einziges Konzept – nicht als lose Deko-Tipps.</p>
    </section>
    <section class="upload-card">
      <input id="room-photo-input" data-testid="room-photo-input" type="file" accept="image/*" capture="environment" class="visually-hidden">
      ${hasImage ? `<div class="photo-preview"><img src="${escapeHtml(state.imageUrl ?? '')}" alt="Ausgewählter Raum"><div class="photo-status"><span>${icon('check')} Foto bereit</span><button data-action="choose-photo">Ändern</button></div></div>` : `<button class="upload-zone" data-action="choose-photo"><div class="camera-orbit">${icon('camera')}</div><strong>Raum fotografieren</strong><span>oder Foto auswählen</span></button>`}
      ${hasImage ? `<div class="image-insight">${icon('sparkle')}<span>${state.isAnalysing ? 'Foto wird gelesen …' : `Erster Eindruck: ${signalLabel(state.signals)}`}</span><small>${escapeHtml(state.fileName)}</small></div>` : ''}
    </section>
    <section class="mode-section">
      <h2>Was soll Oum Wonder tun?</h2>
      <div class="mode-grid">
        <button class="mode-card ${state.mode === 'inspire' ? 'selected' : ''}" data-mode="inspire"><div class="mode-icon">${icon('sparkle')}</div><strong>Überrasch mich</strong><span>Neue Perspektiven, an die du selbst noch nicht gedacht hast.</span><div class="radio-dot" aria-hidden="true"></div></button>
        <button class="mode-card ${state.mode === 'solve' ? 'selected' : ''}" data-mode="solve"><div class="mode-icon">${icon('light')}</div><strong>Gezielt verbessern</strong><span>Beschreibe, was dich stört – wir lösen es als Ganzes.</span><div class="radio-dot" aria-hidden="true"></div></button>
      </div>
    </section>
    <div class="bottom-actions sticky"><button class="primary-button full" data-action="continue" ${!hasImage || state.isAnalysing ? 'disabled' : ''}>Raum neu denken ${icon('arrow')}</button>${!hasImage ? '<small>Füge zuerst ein Foto hinzu.</small>' : ''}</div>`
}

function renderBrief(): string {
  return `${topbar()}
    <section class="brief-header">
      <button class="back-link" data-action="back-start">${icon('back')} Foto & Modus</button>
      <div class="eyebrow">${icon('sparkle')} Fast fertig</div>
      <h1>${state.mode === 'inspire' ? 'Gib mir nur den Rahmen.' : 'Was stört dich am Raum?'}</h1>
      <p>${state.mode === 'inspire' ? 'Oum Wonder übernimmt die kreative Richtung. Du bestimmst nur Raum und Spielraum.' : 'Schreib es so, wie du es jemandem im Raum erklären würdest.'}</p>
    </section>
    <section class="brief-form">
      <div><label class="field-label">Welcher Raum ist das?</label><div class="chip-scroll" role="group" aria-label="Raumtyp">${roomTypes.map((room) => `<button class="chip ${state.roomType === room ? 'active' : ''}" data-room="${room}">${room}</button>`).join('')}</div></div>
      ${state.mode === 'solve' ? `<div class="text-field-wrap"><label for="concern">Was soll besser werden?</label><textarea id="concern" maxlength="500" rows="5" placeholder="Zum Beispiel: Die Küche wirkt dunkel und eng. Ich möchte mehr Licht, mehr Arbeitsfläche und dass sie ruhiger und hochwertiger aussieht.">${escapeHtml(state.concern)}</textarea><small id="concern-count">${state.concern.length}/500</small></div>` : ''}
      <div class="budget-group"><label class="field-label">Wie weit dürfen wir denken?</label><div class="budget-options">${(Object.keys(budgets) as Budget[]).map((budget) => `<button class="budget-card ${state.budget === budget ? 'active' : ''}" data-budget="${budget}"><span>${budgets[budget].title}</span><small>${budgets[budget].sub}</small></button>`).join('')}</div></div>
    </section>
    <div class="bottom-actions sticky"><button class="primary-button full" data-action="generate" ${state.mode === 'solve' && state.concern.trim().length < 4 ? 'disabled' : ''}>${icon('sparkle')} Meine Raumvision erstellen</button>${state.mode === 'solve' && state.concern.trim().length < 4 ? '<small>Beschreibe kurz, was dich stört.</small>' : ''}</div>`
}
...
function renderResult(): string {
  ...
  return `${topbar(true)}
    ...
    <section class="first-steps"><div class="section-heading"><span>So würdest du anfangen</span><small>ohne Fehlkäufe</small></div>${concept.firstSteps.map((item) => `<div class="step-row">${icon('check')}<span>${item}</span></div>`).join('')}</section>
    <div class="bottom-actions result-actions"><button class="secondary-button" data-action="back-to-brief">Variante ändern</button><button class="primary-button" data-action="reset">Neuen Raum starten ${icon('arrow')}</button></div>`
}

function render(): void {
  root.innerHTML = state.step === 'start' ? renderStart() : state.step === 'brief' ? renderBrief() : renderResult()
  bindEvents()
}
...
function bindEvents(): void {
  root.querySelectorAll<HTMLElement>('[data-action]').forEach((element) => {
    element.addEventListener('click', () => {
      ...
      if (action === 'generate') {
        state.concept = createConcept({ mode: state.mode, roomType: state.roomType, concern: state.concern, budget: state.budget, signals: state.signals })
        state.step = 'result'; render(); window.scrollTo(0, 0)
      }
  ...
  root.querySelector<HTMLInputElement>('#room-photo-input')?.addEventListener('change', async (event) => {
    ...
    render()
    state.signals = await analyseImage(file)
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-room]').forEach((button) => button.addEventListener('click', () => {
    state.roomType = button.dataset.room as RoomType
    render()
  }))
  root.querySelectorAll<HTMLButtonElement>('[data-budget]').forEach((button) => button.addEventListener('click', () => {
    state.budget = button.dataset.budget as Budget
    render()
  }))
  root.querySelector<HTMLTextAreaElement>('#concern')?.addEventListener('input', (event) => {
    const textarea = event.currentTarget as HTMLTextAreaElement
    state.concern = textarea.value
    const count = root.querySelector<HTMLElement>('#concern-count')
    if (count) count.textContent = `${state.concern.length}/500`
    const generate = root.querySelector<HTMLButtonElement>('[data-action="generate"]')
    if (generate) generate.disabled = state.concern.trim().length < 4
  })
}

function reset(): void {
  if (state.imageUrl) URL.revokeObjectURL(state.imageUrl)
  Object.assign(state, { step: 'start', imageUrl: null, fileName: '', signals: neutralSignals, mode: 'inspire', roomType: 'Wohnzimmer', budget: 'balanced', concern: '', concept: null, isAnalysing: false })
  render()
  window.scrollTo(0, 0)
}

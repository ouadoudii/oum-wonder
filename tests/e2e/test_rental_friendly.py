...
def run_flow(page, mobile=False):
    ...
    rental = page.locator('[data-rental-friendly]')
    rental.check()
    page.get_by_role('button', name='Küche', exact=True).click()
    assert rental.is_checked()

    page.get_by_role('button', name='Meine Raumvision erstellen').click()
    insight = page.locator('[data-rental-friendly-insight]')
    insight.wait_for()
...
print('Rental-friendly browser tests passed on desktop and Pixel 7.')

...
def run_flow(page, mobile=False):
    ...
    page.get_by_text('Was muss im Alltag funktionieren?').wait_for()
    kids = page.get_by_role('button', name='Kinder', exact=True)
    accessible = page.get_by_role('button', name='Barrierearm', exact=True)
    kids.click()
    accessible.click()
    assert kids.get_attribute('aria-pressed') == 'true'
    assert accessible.get_attribute('aria-pressed') == 'true'

    # Normal brief re-renders preserve the selected everyday needs.
    page.get_by_role('button', name='Küche', exact=True).click()
    assert page.get_by_role('button', name='Kinder', exact=True).get_attribute('aria-pressed') == 'true'
    assert page.get_by_role('button', name='Barrierearm', exact=True).get_attribute('aria-pressed') == 'true'

    page.get_by_role('button', name='Meine Raumvision erstellen').click()
    insight = page.locator('[data-everyday-needs-insight]')
    insight.wait_for()
    text = insight.inner_text()
    assert 'Alltag mitgedacht' in text
    assert 'abgerundete Kanten' in text
    assert 'Breite, freie Laufwege' in text
...
print('Everyday-needs browser tests passed on desktop and Pixel 7.')

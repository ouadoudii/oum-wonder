...
PUBLIC = ROOT / 'public'
SVG = b'''<svg xmlns="http://www.w3.org/2000/svg" width="900" height="700"><rect width="900" height="700" fill="#c8c0b5"/><rect x="80" y="80" width="300" height="420" fill="#f0d8a8"/></svg>'''
...
def run_flow(page, mobile=False):
    ...
    field = page.get_by_label('Maximales Budget in Euro')
    field.fill('900')
    page.get_by_role('button', name='Küche', exact=True).click()
    assert page.get_by_label('Maximales Budget in Euro').input_value() == '900'

    page.get_by_role('button', name='Meine Raumvision erstellen').click()
    insight = page.locator('[data-budget-ceiling-insight]')
    insight.wait_for()
    text = insight.inner_text()
    assert 'Budget auf Quick Wins fokussieren' in text
    assert 'keine verbindliche Kostenschätzung' not in text
...
print('Budget-ceiling browser tests passed on desktop and Pixel 7.')
